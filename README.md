...
В корневой папке проекта хранится виртуальное окружение Python 3.10.10
Для активации достаточно выполнить команду

    source .venv/bin/activate (в bash)

из корневой папки.

Запуск приложения осуществляется из папки bookkeeper/exe/PyBookkeeperApp.py:

    python PyBookkeeperApp.py.

По умолчанию в скрипте приложению передан SQlite-репозиторий, базы данных
хранятся в папке bookkeeper/exe/repos. Для подключения репозитория,
работающего в оперативной памяти, следует передать в repo_factory
